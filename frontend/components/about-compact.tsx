export function AboutCompact() {
  const skills = [
    ...
    { category: "Tools", items: ["Spring", "Spring Boot", "Docker", "Terraform", "Jenkins", "DataDog", "PagerDuty"] },
    { category: "AI/LLM/RAG", items: ["AWS Bedrock", "S3 Vectors", "Claude Code", "Junie", "ChatGPT", "Copilot", "Gemini"] },
  ];
    ...
    <section id="about" className="py-8 px-6 border-b border-border">
      <div className="flex items-center gap-3 mb-4">
        <h2 className="text-lg font-bold text-foreground whitespace-nowrap">
          <span className="text-primary font-mono text-base mr-1">01.</span>
          About Me
        </h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {skills.map((group) => (
          <div key={group.category}>
            <h3 className="text-foreground font-semibold text-sm mb-2">{group.category}</h3>
            <ul className="space-y-0.5 font-mono text-sm text-muted-foreground">
